...
var testData = `gbc-frperg-pubpbyngr-znantrzrag-377-rgbnp
...
var rawData = `gbc-frperg-pubpbyngr-znantrzrag-377-rgbnp
...
function checkRoom(arr) {
    ...
    for (var i = 0; i < array.length; i++) {
        var prime = array[i]
        var keyString = array[i].slice(0, array[i].length - 2).join('')
        var key = getKey(keyString)
        if (key == prime[prime.length - 1]) {
            decypheredString = []
            for (var x = 0; x < keyString.length; x++) {
                var element = keyString[x];
                var value = (parseInt(prime[prime.length - 2])%26)
                decypheredString.push(shiftChar(element,value))
            }
            let roomCode = decypheredString.join('')
            if(roomCode.includes("northpole")){
                console.log(roomCode)
                console.log(prime[prime.length - 2])
            }
...
function getKey(string) {
    var elementArr = []
    var keyString = []
    var elementObj = countChar(string)
    for (var key in elementObj) {
        elementArr.push({char: key, count: elementObj[key].count})
    }
    elementArr = elementArr.sort(function (a, b) {
       if (b.count == a.count) {
           if (a.char < b.char) //sort string ascending
               return -1;
           if (a.char > b.char)
               return 1;
           return 0;
    ...
    for (var index = 0; index < 5; index++) {
        var char = elementArr[index].char;
        keyString.push(char)
    }
...
function groupEqualQuantities(arr) {
    let group = []
    let lastSlice = 0;
    for (var index = 0; index < arr.length - 1; index++) {
        ...
        if (value == arr[index + 1][1]) {
            ...
            let same = arr.slice(lastSlice, index + 1)
            lastSlice = index + 1
            group.push(toKey(same))
        }
    }
    return group.join('')
}
...
function shiftChar(c, value) {
    let letter = c.charCodeAt(0)
    return String.fromCharCode((letter - 97+value) % 26 +97);
}
